...
\end{align*}
不妨设$\tau_{1}>\tau_2$,将$u$代入到第二个方程，则$u$为下解，取足够大的$M$使得$Mv\geq u$，则由上下解方法及至多唯一性，有$u\leq v\leq Mv$，因此式~\eqref{eq11}唯一性成立。\\
...
\]
严格递增便得到$I^*$的唯一性.\\
若$d=0$,根据~\eqref{eq10}可知$S$存在上界，从而对~\eqref{eq1}作$Harnack$不等式，便可知道$I$也有上界，类似上边分析，可得到一样的结果.\\
子情况4:$d_I\to \infty$,由~\eqref{eq3}可知$I$有一致上界，因此$S$也有一致上界，结合椭圆估计及嵌入定理，可以抽出子列使得$(S_n,I_n)\to (S^*,I^*)$在$C^{2}(\overline{\Omega})$意义下，且$I^*$为非负常数，满足以下方程:
\begin{equation}
    \begin{aligned}
    d_S\Delta S^*-\beta S^* f(I^*)+\gamma I^*=0
    \end{aligned}
\end{equation}
这里可以类似Wu-zou的文章，假设是在高风险区域下讨论，能保证$I^*$严格大于0，这里略过，以后再补.\\
$d_I\to 0$的情况，我再挣扎一下，$Wu\-Zou$的文章的证法不能直接使用$Lou$文章的定理，必须保证解存在才能去逼近，$Wu\-Zou$的文章是直接假设如果解不存在就设置为0，这不符合定理要求.
